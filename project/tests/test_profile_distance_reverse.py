...
def test_profile_distance_matches_full_route_length_reversed():
    ...
    url = (
        ...
        '&step_km=60&profile_step_km=30&reverse=1'
    )
    ...
    # Check last sampled distance is close to total_km (within 1%) even when reversed
    ...
    assert abs(last - total_km) / max(1.0, total_km) < 0.01, (
        f'Reversed profile last distance {last:.2f} km deviates from route {total_km:.2f} km')
